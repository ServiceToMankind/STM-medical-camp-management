...
if(isset($_POST['pquery'])){
    $pquery = $_POST['pquery'];
    $url = "https://apis.stmorg.in/medical/records/records?pquery=".$pquery ."&page=".$page;
    $data = get_api_data($url);
    $data = json_decode($data, true);
    $total_pages = $data['total_pages'];
    $data = $data['data'];
}else{
    $url = "https://apis.stmorg.in/medical/records/records?page=".$page;
    $data = get_api_data($url);
    $data = json_decode($data, true);
    $total_pages = $data['total_pages'];
    $data = $data['data'];
                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    ...
                                    for($i=0; $i<count($data); $i++){
                                        ...
                                        ?>
                                    <tr onclick="window.location.href='recordview?id=<?php echo $data[$i]['id']; ?>'">
                                        <td>STMR<?php echo $data[$i]['id']; ?></td>
                                        <td><?php echo $diagdata[0]['name'] ?></td>
                                        <td><?php echo $data[$i]['prescription']; ?></td>
                                        <td><?php echo $ddata[0]['name'] ?></td>
                                        <td><?php echo $data[$i]['last_visit']; ?></td>
                                        <td>
                                            <?php
                                                if($data[$i]['status'] == 1){
                                                    ?>
                                        ...
                                        <td>
                                            <a href="manage_records?rid=<?php echo $data[$i]['id']; ?>"
